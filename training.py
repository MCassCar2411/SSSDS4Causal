...
def generate_samples(file_path_train, config, csv_path, inference_data_path, n_samples, dataset, selected_date=None, csv_substation=False):
    ...
    # Load the dataset (final will have temp, hum, day type, month type and holiday info)
    #Compute avergae profile for each substation 
    if csv_substation:
        data = pd.read_csv(csv_substation)
        #print(data)
        #data = data[data['SPLICENCEA'] =='SPD'].reset_index() #location
        sub_name = data['Network']
        n_customers = data['Customers']     
        start_date = '01/06/2025'
        end_date = '01/07/2025'
        label='Winter'
        date = pd.date_range(start=start_date, end=end_date, freq="30min").date.tolist()

        mean_customer=np.mean(n_customers)

        for i, sub_size in enumerate(n_customers):
            print(i)
            print(sub_size)
            config.train["batch_size"] = int(sub_size)

            calculate_sub_impact(csv_path, dataset=dataset, task="infer")

            date = pd.date_range(start=start_date, end=end_date, freq="30min").date.tolist()
            new_dataset= build_diffusion_loaders(csv_path, config, split="inference", train_data = train_dataset.dataset, selected_date = date) #Selected date?

            #start_date = pd.to_datetime(new_dataset.dataset.data_date[0]).normalize()
            #end_date = pd.to_datetime(new_dataset.dataset.data_date[-1]).normalize() + pd.Timedelta(days=1)
            #date=pd.DataFrame(pd.date_range(start=start_date, end=end_date, freq="30min")) #.iloc[:-1,:]
            

            num_days = len(date) // 48
            if np.isnan(sub_size) or sub_size==0:
                sub=mean_customer    
            aggregate= pd.DataFrame()
            for n in range(n_samples):
                generated_data = diffusion_model.infer(diffusion_model.denoiser, new_dataset, train_dataset, fixed_noise=False, type='Guide')
                generated_dataset = reverse_load(generated_data, train_dataset)

                generated_dataset = generated_dataset.reshape(num_days, sub_size, 48, 1)

        
                generated_dataset = generated_dataset.transpose(0, 2, 1, 3).squeeze(-1)  #B, T, C
                #generated_dataset = generated_dataset.reshape(-1, sub_size)

                B, T, C = generated_dataset.shape
                generated=pd.DataFrame(generated_dataset.reshape(B*T, -1))
  
                aggregated= pd.concat([aggregate, generated], axis=1)

            date=pd.DataFrame(date)
            aggregate= pd.concat([date, aggregated], axis=1, ignore_index=True)
            print(aggregate)
            #save per customer file
            # Save generated data
            output_file = f"data/inference/gen_data/synthetic_data_{sub_name[i]}.csv"
            aggregate.iloc[:-1,:].to_csv(output_file, index=False)
        ...
        end_date = '02/12/2023'
        label='Winter'
        selected_date = pd.date_range(start=start_date, end=end_date, freq="30min").date.tolist()
        print(f"Generating {n_samples} synthetic samples for {n_customers} customers on {np.unique(selected_date)}")
        
        new_dataset=  build_diffusion_loaders(csv_path, config, split="validation", train_data = train_dataset.dataset, selected_date = selected_date) #Selected date?
        num_customers = config.train["batch_size"]
        samples = [] 
        date = pd.date_range(start=start_date, end=end_date, freq="30min").tolist()
        num_days = len(date) // 48
        calculate_sub_impact(csv_path, dataset=dataset, task="infer", selected_date=selected_date)
        for n in range(n_samples):
            # Inference
            generated_data = diffusion_model.infer(diffusion_model.denoiser, new_dataset, train_dataset, fixed_noise=False, type='Guide')
            generated_dataset = reverse_load(generated_data, train_dataset)
        ...
        for i in range(num_customers):
            cust_df = pd.DataFrame({
                ...
                'Load_kW': mean_data_final[:, i]
            })
            cust_df.to_csv(os.path.join(output_folder, f"Customer_{i+1}.csv"), index=False)
        ...
        df_agg_save.to_csv(os.path.join(inference_data_path, "Aggregate_Synthetic.csv"), index=False)
...
def train_diffusion(config, train_loader, dataset):
    ...
    data_values = train_loader.dataset


    optimizer = optim.Adam(model.parameters(), lr=train_config["lr"])
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[int(0.75 * train_config["num_epochs"]), int(0.9 * train_config["num_epochs"])], gamma=0.1)

    lr_scheduler =   torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=train_config["num_epochs"])
    ...
    for epoch in range(train_config["num_epochs"]):
        ...
        for batch in train_loader:
            ...
            conditions = batch["cond_data"].to(config.train["device"]) # (B, num_exogenous, L)

            B, T, C = x0.shape

            t = torch.randint(0, config.diffusion["T"] , (B,), device=config.train["device"])
            #Forward Diffusion
        ...
        print(f" Epoch {epoch+1}/{train_config['num_epochs']} - Train Loss: {avg_train_loss:.4f}, Training Time: {train_time[-1]}s")
        lr_scheduler.step()
...
def evaluate(config, file_path_train, file_path_val, dataset="elektro", seed=42, test_pvalue=False):

    set_global_seed(seed)
    if test_pvalue:
        evaluate_p(config, file_path_train, file_path_val, config.train["device"], dataset=dataset)
        
    models= {"WGAN", "SSSDS4+causal", "SSSDS4"} #, "CREST", "elexon"}
    #models = {"SSSDS4+causal", "SSSDS4"}
    admd_stats, power_values, other_scores, power_metrics, all_real_stats, all_gen_stats, aggregate_feat = evaluate_models(config, file_path_val, file_path_train, config.input_dim, config.train["device"], models, best_epoch=config.eval["epoch_id"], seed=seed, dataset=dataset)
...
def evaluate_models(config, file_path_val, file_path_train, input_dim, device, losses= {"mse"}, n_samples=500, best_epoch=200, seed=42, dataset="elektro"):
    ...
    val_loader = build_diffusion_loaders(file_path_val, config, split="validation", train_data=train_loader.dataset)
    ...
    for batch in val_loader:
        ...
        x_t = torch.randn((x0.shape[0], x0.shape[1], x0.shape[2]), device=device)  # Start from pure noise
        x_t_val_list.append(x_t)
    
    
    x_t_data = torch.cat(x_t_val_list, axis=0)
    ...
    for loss_name in losses:
        ...
        if loss_name=="SSSDS4+causal" or "SSSDS4":
            ...
            if  loss_name == "SSSDS4":
                with torch.no_grad():
                    ...
                    generated_dataset = pd.read_csv(save_dir + "_Diff_synth.csv", index_col=0)
                ...
                with torch.set_grad_enabled(True):
                    ...
                    """
                    generated_dataset = pd.read_csv(save_dir + "_Diff_Causal_synth.csv", index_col=0)
        ...
        if loss_name == "WGAN":
            ...
            gen = Generator_linear(latent_s=config_GAN['latent_s'], cond_in=config_GAN['cond_in'], in_size=config_GAN['in_size'], gen_w=config_GAN['gen_w'], gen_l=config_GAN['gen_l'], gpu=device)
            ...
            # Sample MODEL
            generated_data = build_gan_scenarios(val_loader=val_dataset_diff, gen=gen, max=max_cap, gpu=device, tag='load', non_null_indexes=non_null_indexes)
        
            print(generated_data.shape)

            generated_dataset = reverse_load(generated_data.reshape(num_days, T, size).transpose(0, 2, 1), train_dataset_diff) #C*T, B
            print(generated_dataset.shape)            
            # split into customers and days
        ...
        #Household level 
        print("These are real stats")
        real_stats_h = compute_power_stats(real_dataset, label="real", level='household')
        print("These are synth stats")
        gen_stats_h = compute_power_stats(generated_dataset, label="synth", level='household')
...
def evaluate_p(config, file_path_train, file_path_val, device, dataset="elektro"):
    ...
    with torch.set_grad_enabled(True):
        for p_val in p_arr:
            ...
            if os.path.exists(os.path.join(os.getcwd(), save_dir, f'Diff_Causal_synth_{p_val}.csv')):
                generated_dataset = pd.read_csv(save_dir + f"/Diff_Causal_synth_{p_val}.csv", index_col=0)
                   
                generated_dataset = generated_dataset.values.reshape(num_days, T, size)
               

            else:               
                generated_data = diffusion_model.infer(diffusion_model.denoiser, val_loader, train_loader, fixed_noise=True, type='Guide', p = p_val) #replace by loading the same data
                generated_dataset = reverse_load(generated_data, train_loader)
                    
                # split into customers and days
                generated_dataset = generated_dataset.reshape(num_days, size, T, 1)
                generated_dataset = generated_dataset.transpose(0, 2, 1, 3).squeeze(-1) # B, T, C
                pd.DataFrame(generated_dataset.reshape(-1, C)).to_csv(save_dir + f"/Diff_Causal_synth_{p_val}.csv")
