...
from metrics import reverse_load_train
from network import SSSDS4
...
class Diffusion:
    def __init__(self, config):
        ...
        )  # alpha for previous time step
        self.posterior_mean_coef1 = (
            self.beta
            * torch.sqrt(self.alphas_cumprod_prev)
            / (1.0 - self.alphas_cumprod)
        )  # first part of equation with x0
        ...
        self.posterior_variance = (
            self.beta * (1.0 - self.alphas_cumprod_prev) / (1.0 - self.alphas_cumprod)
        )  ##third part with error term
        self.model_log_variance = torch.log(
            torch.clamp(self.posterior_variance, min=1e-20)
        )  # Avoid log(0) log variance for stability, exponentiated during sampling
        ...
        self.sqrt_recipm1_alphas_cumprod = self.sqrt_recipm1_alphas_cumprod.to(
            self.device
        )
    ...
    def predict_start_from_noise(self, x_t, noise_pred, t):
        sqrt_recip_alphas_cumprod_t = self.sqrt_recip_alphas_cumprod[t].reshape(
            -1, 1, 1
        )
        sqrt_recipm1_alphas_cumprod_t = self.sqrt_recipm1_alphas_cumprod[t].reshape(
            -1, 1, 1
        )
    ...
    def reverse_diffusion(self, x_t, pred, t, timestep):
        ...
        x_0_pred = self.predict_start_from_noise(
            x_t, pred, t
        )  # Estimate clean x0 from noisy xt and predicted noise
    ...
    def infer(
        self, denoiser, dataset, real_dataset=None, fixed_noise=False, type="DDPM", p=10
    ):
        generated_data = []
        if fixed_noise:
        ...
        for batch in dataset:
            ...
            if fixed_noise:
                x_t = x_t_fixed[j : j + B]  # type: ignore
                j += B  # type: ignore

            else:
                x_t = torch.randn(
                    (B, self.input_dim, self.seq_length), device=self.device
            ...
            if type == "DDPM":
                for i in reversed(range(self.diffusion_timesteps)):
                    ...
                    x_t = self.reverse_diffusion(
                        x_t, noise_pred, t_tensor, i
                    )  # one step and stochastis and non-differentiable
                    x_t = x_t.detach().cuda()
                ...
                for i in reversed(range(self.diffusion_timesteps)):
                    ...
                    # Combine the losses
                    loss = torch.mean((agg_generated - agg_real) ** 2) + torch.mean(
                        (eve_ramp - eve_ramp_r) ** 2
                    )
