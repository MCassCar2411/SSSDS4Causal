...
def assemble_feeder_data(dir, csv_directory):
    ...
    cols_to_fix = [
        'direct_radiation_W/m2', 'diffuse_radiation_W/m2', 'precipitation_kg/m2', 
        'snow_height_m', 'humidity_kg/kg', 'max_wind_gust_m/s', 
        'meridional_wind_m/s', 'zonal_wind_m/s'
    ]
    ...
    HP_causality = feeder[['feeder', 'Date', 'Power', 'direct_radiation_W_m2', 'diffuse_radiation_W_m2', 'air_temperature_C', 'precipitation_kg_m2', 'snow_height_m', 'humidity_kg_kg', 'max_wind_gust_m_s', 'meridional_wind_m_s', 'zonal_wind_m_s', 'Day_of_Week', 'Month', 'Holiday', 'Time',  'housing_units_count', 'industry_and_commerce_count',	'storage_heaters_kW', 'heat_pumps_kW', 'electric_heaters_kW', 'hot_water_tanks_kW',	'flow-type_heaters_kW',	'other_consumers_kW', 'batteries_kW', 'PV_systems_kW']]
    ...
    for f in filter_f:
        ...
        HP_causality_f = HP_causality[HP_causality['feeder']==f].reset_index(drop =True)
    
        HP_causality_f['hour'] = pd.to_datetime(HP_causality_f['Time'], format='%H:%M:%S').dt.hour
        HP_causality_f['month'] = pd.to_datetime(HP_causality_f['Date'], format='%Y-%m-%d').dt.month
        HP_causality_f['day'] = pd.to_datetime(HP_causality_f['Date'], format='%Y-%m-%d').dt.dayofweek 
    
        confounders = ['direct_radiation_W_m2', 'diffuse_radiation_W_m2', 'precipitation_kg_m2', 'snow_height_m', 'humidity_kg_kg', 'max_wind_gust_m_s', 'meridional_wind_m_s', 'zonal_wind_m_s', 'day', 'month', 'Holiday', 'hour', 'PV_systems_kW']
        treatment = 'air_temperature_C'
        outcome = 'Power'
        control =  'heat_pumps_kW'
        T = HP_causality_f[treatment].values # Treatment: 0 (warm) to 1 (cold)
        Y = HP_causality_f[outcome].values #outcome
        W = HP_causality_f[confounders].values # Covariates / confounders

        #Manual CATE
        confounders_t= ['direct_radiation_W_m2', 'diffuse_radiation_W_m2', 'precipitation_kg_m2', 'snow_height_m', 'humidity_kg_kg', 'max_wind_gust_m_s', 'meridional_wind_m_s', 'zonal_wind_m_s', 'day', 'month', 'Holiday', 'hour',
              'PV_systems_kW']
        confounders_y =  ['direct_radiation_W_m2', 'diffuse_radiation_W_m2', 'precipitation_kg_m2', 'snow_height_m', 'humidity_kg_kg', 'max_wind_gust_m_s', 'meridional_wind_m_s', 'zonal_wind_m_s', 'day', 'month', 'Holiday', 'hour',
              'PV_systems_kW']
        eq= "y_res ~ t_res"
        starmap_parameters.append((HP_causality_f, f, confounders_t, confounders_y, treatment, outcome, control, eq))
...
def run_cross_feeder(causal_path, csv_directory):
    ...
    all_feeder = all_feeder[['feeder', 'housing_units_count', 'industry_and_commerce_count', 'heat_pumps_kW']]
    ...
    for file in os.listdir(csv_directory):
        ...
        for name, res in all_results_dfs.items():
            if not res.empty:
                ...
                res = res.groupby('mean_t').mean().reset_index() # Average across windows of same mean temp to get clearer trend, then reindex for plotting
                ...
                if not (res['control'].nunique() == 1): 
                    ...
                    for cap in sorted(capacities, reverse=True):
                        ...
                        cold_windows_cap = at_cap[at_cap['mean_t'] <= cold_threshold_cap]
                        robust_cold_cap = cold_windows_cap[cold_windows_cap['rv'] >= 0.1]
                        if not robust_cold_cap.empty:
                            print(f"Using cold windows at capacity {cap} instead.")
                            feeder = all_feeder[(all_feeder['feeder'] == int(name)) & (all_feeder['heat_pumps_kW'] == round(float(cap), 2))]

                            feeder_size = int(feeder[['housing_units_count']].max()) + int(feeder[['industry_and_commerce_count']].max()) 
                    ...
                    rv_mean      = robust_cold['rv'].mean() if 'rv' in robust_cold.columns else np.nan
                    print(f"Feeder {name} - Feeder Size {feeder_size} -Worst-case CATE: {worst_cate}, Cold CATE p10: {cold_p10}, RV mean: {rv_mean}, Number of cold windows: {len(robust_cold)}")
...
def model_robustness(file_path, treatment, controls, dataset):
    ...
    for config in robustness_configs:
        ...
        rmse_t, rmse_y, score, params_t, params_y = residualize_data(df, 'Aggregate', controls, controls, treatment, config['n_folds'], config['algo'], algo_search = True)

        scores[config['algo']] = {'rmse_t': rmse_t, 'rmse_y': rmse_y, 'total': score, 'params_t': params_t, 'params_y': params_y}
    ...
    master_df.to_csv(f"results/causality/{dataset}/master_robustness_results.csv", index=True)
...
def causality_analysis(df, treatment, control_set, customer_size=None, dataset="elektro", n_iterations=1, save=True, sensitivity=True):
    ...
    for treatment in treatment_possibilities:
        ...
        for m in available_stratums:
            ...
            for hour in range(24):
                ...
                for start in range(0, len(subset) - window_size + 1, step_size): #overlapping by 5000-500 = 4500 (0-4999, 500-5499)
                    ...
                    with tqdm(total=n_iterations, desc=f'Window {start // step_size + 1}/{total_windows}', leave=False) as pbar:
                        ...
                        for _ in range(n_iterations):
                            ...
                            df_residualised = residualize_data(random_subset, 'agg', controls, controls, treatment, 5, best, params_t=params_t, params_y=params_y)
                            ...
                            if sensitivity:
                                ...
                                results.append({
                                    ...
                                    "lower_confidence_bound": b_res['lower_confidence_bound'],
                                    "upper_confidence_bound": b_res['upper_confidence_bound']
        ...
        if save:
            results_df.to_csv(f"results/causality/{dataset}/results_causal_{treatment}_per_hour_day_{customer_size}.csv", index=False)
    
       

            results_df = pd.read_csv(f"results/causality/{dataset}/results_causal_{treatment}_per_hour_day_{customer_size}.csv")
...
def estimate_CATE_control(df, f, conf_t, conf_y, treatment, outcome, control, eq):
    ...
    if not (all_cap[0] == 0.0 and cap_numb == 1):
        for cap in all_cap:
            ...
            df_cap = df_cap.sort_values(by=treatment) #destroys any temporal information here
            #print(len(df_cap))
            ...
            # Sliding window analysis
            for start in range(0, len(df_cap) - window_size + 1, step_size): #overlapping by 5000-500 = 4500 (0-4999, 500-5499)
                ...
                for _ in range(n_iterations):
                    random_subset = window_data.sample(n=min(len(window_data), window_size), replace=True)
                    df_residualized = residualize_data(random_subset, y, conf_t, conf_y, treatment)
    ...
    results_df.to_csv(f"results/causality/uq_results_causal_temp_hp_{f}.csv", index=False)
        
    return results_df


def estimate_CATE_general(df, feeder, conf_t, conf_y, treatment, outcome, eq, type="nonlinear"):
    window_size = 10000
    step_size = 1000
...
def refute_random_common_cause(dataset, num_simulations=50, random_state=42, run=True, df=None):
    ...
    og_df = pd.read_csv(f"{path}/results_causal_temperature_per_hour_day_1000_500.csv") #change name later
    
    if run: 
        df = pd.read_csv(f"{df}") #change name later
        treatment = ['temperature']
        controls = ['temperature', 'wind_speed', 'humidity', 'clear_sky_ghi', 'rainfall', 'month', 'Holiday', 'w_rand']
        sims=[]
    
        for s in range(num_simulations):
            print(f"This is simulation {s} -----------------")
            new_df=df.assign(w_rand=random_state.normal(size=df.shape[0]))
            new_effect =  causality_analysis(new_df, treatment, controls, dataset=dataset, save=False, sensitivity=False)
            new_effect['sim_id'] = s
            sims.append(new_effect)
 
        new_estimates = pd.concat(sims, ignore_index=True)

        new_estimates.to_csv(f"results/causality/{dataset}/results_RCC_{num_simulations}.csv")
    
    new_estimates = pd.read_csv(f"results/causality/{dataset}/results_RCC_{num_simulations}.csv")

    sim_estimates = (new_estimates
                     .groupby(['label', 'mean_t', 'sim_id'])['cate_linear']
                     .mean().reset_index())
    #breakpoint()
    
    #original point estimate with bootstrap per (feature, label)
    og_stats = (og_df
                .groupby(['label', 'mean_t'])['cate_linear'] #not grouped by tempertaure where bootstraps are applied?
    ...
    for (label, t), grp in sim_estimates.groupby(['label', 'mean_t']):
        ...
        #Extract sim values
        sims_arr = grp['cate_linear'].values #significance test needs the distribution of simulated estimates per window, not their average
...
def test_significance(estimate, simulations, significance_level = 0.05):
    ...
    if num_simulations >= 100:  # Bootstrappin 
        ...
        half_p = np.mean((sims > estimate)
                  + 0.5 * (sims == estimate))
        p_value = 2 * min(half_p, 1 - half_p)
        #p_value = max(p_value, 1.0 / (num_simulations + 1)) #?
...
def filter_robust_estimates(df, dataset, run):
    ...
    print(len(df))
    rcc_res = refute_random_common_cause(dataset, num_simulations=100, random_state=42, run=run)
    ...
    df['is_passed'] =  (df['lower_confidence_bound'].round(1) > 0) & (df['upper_confidence_bound'].round(1) > 0) | (df['lower_confidence_bound'].round(1) < 0) & (df['upper_confidence_bound'].round(1) < 0)
    print(df[df['is_passed'] == False])
    pass_sens = df[df['is_passed'] == True] #32, 3
    print(len(df) - len(pass_sens))

    #merge
    pass_sens = pass_sens.merge(pass_rcc[['label', 'mean_t']], on=['label', 'mean_t'], how='inner')
    print(len(pass_sens))
...
def calculate_sub_impact(val_path, dataset, cal_path = None, task="val", selected_date=None):
    #cate_df = pd.read_csv("results/causality/results_causal_temp_per_hour_day_stable.csv")
    cate_df = pd.read_csv(f"results/causality/{dataset}/results_causal_temperature_per_hour_day_hour.csv")
    ...
    if task == "val":
        ...
        val_df.drop(columns=['target_load', 'base_load', 'upper_ci', 'lower_ci'], inplace=True, errors='ignore')
    else:
    ...
    #cal_df['Aggregate'] = cal_df['Power(kW)']
    ...
    if task == "infer":
        ...
        mape = mean_absolute_percentage_error(val_df["target_load"], val_df["Aggregate"])
        print(f"Your Error %: {mape:.2f}%")
        ...
        if selected_date:
            val_df = val_df[((pd.to_datetime(val_df['Date'], dayfirst=True) >= str(selected_date[0])) & \
                    (pd.to_datetime(val_df['Date'], dayfirst=True) < str(selected_date[-1])))].reset_index(drop=True)

        plt.figure(figsize=(12, 6))

        # Base Load 
        plt.plot(val_df.index, val_df['base_load'], 
                 color='black', label='Base Load', alpha=0.6)

        # Real Aggregate
        plt.plot(val_df.index, val_df['Aggregate'], 
                     color='red', label='Aggregate', alpha=0.6)

        # Prediction 
        plt.plot(val_df.index, val_df['target_load'], 
                 color='blue', linewidth=1.5, label='Predicted Load')
        ...
        plt.tight_layout()
        plt.savefig(f'results/causality/{dataset}/causal_load_estimate_winter.png', dpi=300)
        #plt.show()
    else:
        val_df = pd.concat([val_df, res_df], axis=1).reset_index(drop=True)
...
def get_guidance_function(results_df):
    ...
    for label in unique_labels:
        ...
        grouped['cate_lower_ci'] = subset.groupby('mean_t')['cate_linear'].quantile(0.05)
        grouped['cate_upper_ci'] = subset.groupby('mean_t')['cate_linear'].quantile(0.95)
        ...
        """
        sigma_smoothing = 1
        grouped['mean'] = gaussian_filter1d(grouped['mean'], sigma=sigma_smoothing)
        grouped['cate_lower_ci'] = gaussian_filter1d(grouped['cate_lower_ci'], sigma=sigma_smoothing)
        grouped['cate_upper_ci'] = gaussian_filter1d(grouped['cate_upper_ci'], sigma=sigma_smoothing)
        

        grouped['load_mean'] = gaussian_filter1d(grouped['load_mean'], sigma=sigma_smoothing)
        grouped['load_lower_ci'] = gaussian_filter1d(grouped['load_lower_ci'], sigma=sigma_smoothing)
        grouped['load_upper_ci'] = gaussian_filter1d(grouped['load_upper_ci'], sigma=sigma_smoothing)


        grouped['temp_mean'] = gaussian_filter1d(grouped['temp_mean'], sigma=sigma_smoothing)
        grouped['temp_lower_ci'] = gaussian_filter1d(grouped['temp_lower_ci'], sigma=sigma_smoothing)
        grouped['temp_upper_ci'] = gaussian_filter1d(grouped['temp_upper_ci'], sigma=sigma_smoothing)
        """
...
def select_model(algo, params=None):
    params = params or {}
    if algo == 'lgbm':
        return lgb.LGBMRegressor(objective='regression', n_estimators=500,
    ...
    if algo == 'rf':
        return RandomForestRegressor(n_estimators=500, random_state=42,
                                     n_jobs=-1, **params)
    if algo == 'xgb':
        return xgb.XGBRegressor(objective='reg:squarederror', n_estimators=500,
                                random_state=42, n_jobs=-1, **params)

def residualize_variable_kfold(data, y_var, x_vars, n_folds=5, algo='lgbm', params=None):
    ...
    if params is None:
         if algo == 'lgbm':
             param = {
                ...
                'min_child_samples': [15, 40, 80],  # Very risky for N=1000, but sensitive to signal
         ...
         search = RandomizedSearchCV(select_model(algo), param,
                                    n_iter=15, cv=5,
    ...
    kf = KFold(n_splits=n_folds, shuffle=True, random_state=42) #cross validation for robustness
...
# Residualize data
def residualize_data(data, y, conf_t, conf_y, treatment, n_folds=5, algo='lgbm', algo_search = False, params_t=None, params_y=None):
...
def fit_residualized_model(data, conf, eq="y_res ~ t_res"):
    ...
    scale_residualized = pipeline_residualized.named_steps['scaler'].scale_ #find 1 std dev, change in load per 1 SD in treatment
    ...
    residual_df= pd.DataFrame({
        "y_res": data[['agg_resid']].reset_index(drop=True).iloc[:,0], # Should average across split indexes
    ...
    if 't_res' not in ols_model.bse or ols_model.bse['t_res'] == 0:
        return {
            ...
            'rv': np.nan, 'rva': np.nan, 'partial_r2': 0
        }
    ...
    #predictions['bounds'] = sens.bounds  # full DataFrame if you want kd breakdown
        ...
        predictions['rv'], predictions['rva'], predictions['partial_r2'] = np.nan, np.nan, 0
