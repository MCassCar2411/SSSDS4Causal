...
def select_model(algo, params=None):
    ...
    if algo == "lgbm":
        ...
        return lgb.LGBMRegressor(objective="regression", n_estimators=500, random_state=42, n_jobs=-1, **params)
    if algo == "rf":
        from sklearn.ensemble import RandomForestRegressor
 
        return RandomForestRegressor(n_estimators=500, random_state=42, n_jobs=-1, **params)
    if algo == "xgb":
        import xgboost as xgb
 
        return xgb.XGBRegressor(objective="reg:squarederror", n_estimators=500, random_state=42, n_jobs=-1, **params)
