...
def sensitivity_analysis(
    ...
    cv = KFold(
        n_splits=num_splits, shuffle=shuffle_data, random_state=shuffle_random_seed
    )
    ...
    nu_2 = np.mean(
        t_resid**2
    )  # E[T_res^2]  (== E[alpha_s^-1], since alpha_s = T_res/nu_2)
    sigma_2 = np.mean(
        residualized_outcome_second_stage**2
    )  # E[(Y_res - theta_s*T_res)^2]
    ...
    if benchmarking:
        delta_r2_y_wj, delta_r2t_wj = compute_r2diff_benchmarking_covariates(
            features=features,
            T=T,
            Y=Y,
        ...
        if r2tu_w >= 1:
            r2tu_w = 1 - 1e-8
            logger.warning(
                "r2tu_w > 1 (try a lower frac_strength_treatment); clipped to 1."
            )
    ...
    RV, plot_type = calculate_robustness_value(
        alpha=None,
        S2=S2,
        S=S,
        theta_s=theta_s,
    ...
    RV_alpha, _ = calculate_robustness_value(
    ...
    if plot == True:
        ...
        plot_contour(
            ...
            benchmark_common_causes,
            results,
            neyman_orthogonal_score_theta,
            sigma_2,
            neyman_orthogonal_score_treatment,
            nu_2,
            r2tu_w,
            r2yu_tw,
            plot_type=plot_type,
...
def calculate_robustness_value(
    alpha,
    S2,
    S,
    theta_s,
...
def get_confidence_levels(
    ...
    σ2 = E[(Y - E[Y | X, T]) ^ 2] (expected value of residual outcome)
    ...
    if significance_level is not None:
        phi_lower, phi_upper = get_phi_lower_upper(
            Cg,
            Calpha,
            S,
            neyman_orthogonal_score_theta,
...
def get_phi_lower_upper(
    ...
    bounds_estimator_upper = neyman_orthogonal_score_theta + (
        (Cg * Calpha) / (2 * S)
    ) * (
        sigma_2 * neyman_orthogonal_score_treatment
        + nu_2 * neyman_orthogonal_score_treatment
    )
    bounds_estimator_lower = neyman_orthogonal_score_theta - (
        (Cg * Calpha) / (2 * S)
    ) * (
        sigma_2 * neyman_orthogonal_score_treatment
        + nu_2 * neyman_orthogonal_score_treatment
...
def compute_r2diff_benchmarking_covariates(
    ...
    r2t_w_j = get_regression_r2(
        X=W_j, Y=T, split_indices=split_indices, algo=algo, params=params_t
    )
    ...
    r2y_w_j = get_regression_r2(
        X=T_W_j, Y=Y, split_indices=split_indices, algo=algo, params=params_y
    )
...
def plot_contour(
    ...
    if (
        critical_value >= contour_values.min()
        and critical_value <= contour_values.max()
        and plot_type != "bias"
    ):
        contour_plot = ax.contour(
            r2tu_w,
    ...
    if benchmarking:
        if frac_strength_treatment == frac_strength_outcome:
            ...
            signs = (
                str(round(frac_strength_treatment, 2))
                + "/"
                + str(round(frac_strength_outcome, 2))
            )
        label = (
            signs
            + " X "
            + str(benchmark_common_causes)
            + " ({:1.2f}) ".format(results[plot_type][0])
        )
        ax.scatter(r2tu_w_point, r2yu_tw_point, color="red", marker="^", label=label)
...
def estimate_CATE_general(
    df, feeder, conf_t, conf_y, treatment, outcome, type="nonlinear"
):
    window_size = 10000
    step_size = 1000
        ...
        with tqdm(
            ...
            for _ in range(n_iterations):
                ...
                analyser = non_parametric_sensitivity_analysis(
                    ...
                    observed_common_causes=random_subset[
                        [
                            "wind_speed",
                            "humidity",
                            "clear_sky_ghi",
...
if __name__ == "__main__":
    ...
    estimate_CATE_general(
        df, "nps", conf_t, conf_y, treatment, outcome, type="nonlinear"
    )
