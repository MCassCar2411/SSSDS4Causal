...
def calc_diffusion_step_embedding(diffusion_steps, diffusion_step_embed_dim_in):
    ...
    # We also move the arange creation directly to the device to avoid extra copies
    _embed = torch.exp(torch.arange(half_dim, device=device) * -_embed)
    
    _embed = diffusion_steps[:, None] * _embed[None, :]
    diffusion_step_embed = torch.cat((_embed.sin(), _embed.cos()), 1)
...
class Residual_group(nn.Module):
    ...
    def forward(self, noise, conditional, diffusion_steps):
...
class SSSDS4(nn.Module):
    def __init__(self, config):
        ...
        cond_channel = config_model["cond_channel"]
        res_channels=config_model["res_channels"]
        skip_channels=config_model["skip_channels"]
        out_channels=config_model["out_channels"] # Output shape matches Input
        num_res_layers=config_model["num_res_layers"]
        diffusion_step_embed_dim_in=config_model["diffusion_step_embed_dim_in"]
        diffusion_step_embed_dim_mid=config_model["diffusion_step_embed_dim_mid"]
        diffusion_step_embed_dim_out=config_model["diffusion_step_embed_dim_out"]
        s4_lmax=config_model["s4_lmax"]
        s4_d_state=config_model["s4_d_state"]
        s4_dropout=config_model["s4_dropout"]
        s4_bidirectional=config_model["s4_bidirectional"]
        s4_layernorm=config_model["s4_layernorm"]

        self.init_conv = nn.Sequential(Conv(in_channels, res_channels, kernel_size=1), nn.ReLU()) #Noisy load 
        
        #Noisy Load + Condition
        ...
        #Project back to Noisy load form hidden dim
