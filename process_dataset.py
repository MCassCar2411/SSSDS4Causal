...
def extract_calendar_features(data, schema):
    ...
    try:
        ...
        data["datetime"] = pd.to_datetime(
            data[schema.date].astype(str) + " " + data[schema.time].astype(str),
            format="%Y-%m-%d %H:%M:%S",
        )  # %Y-%m-%d or
    except:
    ...
    dow = (
        data["datetime"].dt.dayofweek.astype(int).values.reshape(-1, 1)
    )  # 0=Monday, 6=Sunday

    encoder = OneHotEncoder(
        categories=[range(7)], sparse_output=False, dtype=np.float32
    )
    ...
    encoder = OneHotEncoder(
        categories=[range(1, 13)], sparse_output=False, dtype=np.float32
    )
...
class DiffusionDataset(Dataset):
    def __init__(
        ...
        exog_col_w = [
            ...
            self.sch.wind,
            self.sch.rain,
            self.sch.humidity,  # Continuous (Weather)
        ]
        ...
        if split == "train":
            ...
            if normalisation == "lognorm":
                ...
                target_data = self.minmax_norm_train(
                    target_data, self.max_load, self.min_load
                )
            ...
            self.windows_target = make_windows(
                target_data[:, np.newaxis], seq_len, stride
            )
            ...
            target_data = np.concatenate(customer_data).astype(
                np.float32
            ...
            self.windows_agg = make_windows(
                data[self.sch.aggregate].values.astype(np.float32), seq_len, stride
            )
            self.windows_target = make_windows(
                target_data[:, np.newaxis], seq_len, stride
            )
            ...
            exog_weather = self.max_norm(exog_weather, self.exog_max)
            self.windows_agg = make_windows(
                data[self.sch.aggregate_inf].values.astype(np.float32), seq_len, stride
            )
...
def build_diffusion_loaders(
    ...
    )  # change split for val
    dataloader_diff = DataLoader(
        dataset_diff, config.train["batch_size"], shuffle=(split == "train")
    )  # change shuffle and drop_lat
...
def reverse_load(data, data_real):
    ...
    if data_real.normalisation == "lognorm":
        ...
        data = (
            data * (max_load - min_load + 1e-8) + min_load
    ...
    for c in range(len(num_targets)):  # days
        for j in range(num_targets[c]):  # [152, 151..158]
            for i in range(seq_len):  # timestep
                target_data[i + day] = customer_data[i + (c * seq_len)][
                    j
                ]  # for every timestep save every customer column
            day += seq_len
