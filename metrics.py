...
def wd(n_samples, real_dataset, generated_dataset, level='agg', seeds=4, seed=42):
    ...
    if level == 'agg':
        ...
        synth = generated_dataset.transpose(0, 2, 1).reshape(-1, generated_dataset.shape[1])
...
def rmse_mae_score(real_dataset, generated_dataset, level='agg', n_samples=500, seeds=4, seed=42):
    ...
    for i in range(seeds):
        ...
        mae_list.append(mae)
        print(f'Seed: {i}\tRMSE: {str(rmse).replace(".", ",")}\tMAE: {str(mae).replace(".", ",")}')
...
def compute_power_stats(data, label, level='household'):
    ...
    if level == 'aggregate':
        ...
        data_hourly = data.reshape(data.shape[0], 24, 2, data.shape[2]).mean(axis=2) # (B, 24, C)
...
def power_stat_distances(stats_real, stats_gen):
    distances = {
        'mean_diff_overall': abs(stats_real['overall_mean'] - stats_gen['overall_mean']),
        'peak_diff_overall': abs(stats_real['overall_peak'] - stats_gen['overall_peak']),
        'hourly_mean_rmse': np.sqrt(((stats_real['hourly_mean'] - stats_gen['hourly_mean'])**2).mean()),
        'hourly_peak_rmse': np.sqrt(((stats_real['hourly_peak'] - stats_gen['hourly_peak'])**2).mean()),
        'daily_mean_rmse': np.sqrt(((stats_real['daily_mean'] - stats_gen['daily_mean'])**2).mean()),
        'daily_peak_rmse': np.sqrt(((stats_real['daily_peak'] - stats_gen['daily_peak'])**2).mean())
    }
    return distances
...
def normalise_metrics(all_metrics):
    ...
    metric_min = {metric: np.min(values) for metric, values in all_values_per_metric.items()}
    metric_max = {metric: np.max(values) for metric, values in all_values_per_metric.items()}
...
def measure_peaks(data, C=158, label="real"):
    ...
    night_min_idx = np.argmin(data[:, 0:night_end], axis=1)  # window starts at 0 -> already absolute
